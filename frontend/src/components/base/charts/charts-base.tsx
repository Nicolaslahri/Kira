...
// Untitled UI chart helpers (legend / tooltip / active-dot), vendored verbatim
// from the starter kit. The actual charts are composed with recharts directly
// (PieChart, BarChart, …) styled with these helpers + UUI tokens.

/** Selects evenly spaced items from an array (for x-axis label thinning). */
...
export const ChartTooltipContent = ({ active, payload, label, isRadialChart, isPieChart, formatter, labelFormatter }: ChartTooltipContentProps) => {
    ...
    // If it's a single data point, we use the value as the title and the name as the secondary title.
