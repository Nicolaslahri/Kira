...
async def _resolve_trash_root(session: AsyncSession) -> Path | None:
    """Same resolution the rename pipeline uses: `rename.trash_dir` override,
    else `<library_root>/.kira-trash`. None when no library root is set."""
    def _val(row):
...
async def list_trash(session: AsyncSession = Depends(get_session)) -> dict:
    root = await _resolve_trash_root(session)
    if root is None or not root.exists():
        return {"root": str(root) if root else None, "items": [], "total_bytes": 0}
    ...
    def _scan() -> dict:
        ...
        return {"root": str(root), "items": items, "total_bytes": total}
...
async def empty_trash(session: AsyncSession = Depends(get_session)) -> dict:
    ...
    logger.info("trash: emptied %d item(s)", n)
    return {"deleted": n}


def purge_old_trash(trash_root: Path, retention_days: int) -> int:
    """Delete trash items older than `retention_days` (by mtime). 0/negative =
